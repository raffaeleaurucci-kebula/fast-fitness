...
interface ProfitWeekReturn {
  loading: boolean;
  error: string | null;
  getSubProfitWeek: () => Promise<ProfitWeekOut | null>;
}

export function useGetSubProfitByWeek(): ProfitWeekReturn {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const getSubProfitWeek = async (): Promise<ProfitWeekOut | null> => {
    setLoading(true);
    setError(null);

    try {
      const res = await fetch(`${API_BASE}/subscriptions/profit/week`, {
        method: "GET",
        headers: {
  ...
  return { loading, error, getSubProfitWeek: getSubProfitWeek };
}