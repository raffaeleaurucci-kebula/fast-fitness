...
interface UseUpdateCardExerciseReturn {
  loading: boolean;
  error: string | null;
  updateCardExercise: (
    cardId: number,
...
export function useUpdateCardExercise(): UseUpdateCardExerciseReturn {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const updateCardExercise = async (
    cardId: number,
    cardExerciseId: number,
    payload: TrainingCardExerciseIn
  ): Promise<TrainingCardExerciseOut | null> => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.detail ?? "Errore durante l'aggiornamento dell'esercizio.");
  ...
  return { loading, error, updateCardExercise };
}