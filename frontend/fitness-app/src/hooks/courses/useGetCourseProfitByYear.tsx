...
interface ProfitYearReturn {
  loading: boolean;
  error: string | null;
  getCourseProfitYear: () => Promise<ProfitYearOut | null>;
}

export function useGetCourseProfitByYear(): ProfitYearReturn {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const getCourseProfitYear = async (): Promise<ProfitYearOut | null> => {
    setLoading(true);
    setError(null);
  ...
  return { loading, error, getCourseProfitYear: getCourseProfitYear };
}